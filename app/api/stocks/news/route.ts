...
function deriveTags(input: { symbol: string; headline?: string; summary?: string; source?: string }) {
  const text = `${input.symbol} ${input.headline || ""} ${input.summary || ""} ${input.source || ""}`.toLowerCase()
  const tags = new Set<string>()

  const addIf = (tag: string, patterns: Array<string | RegExp>) => {
    if (patterns.some((p) => (typeof p === "string" ? text.includes(p) : p.test(text)))) tags.add(tag)
  }

  addIf("AI", ["ai", "artificial intelligence", "openai", "llm", "chatgpt", "copilot", "gpu", "nvidia"])
  addIf("Cloud", ["cloud", "azure", "aws", "gcp", "data center", "datacenter"])
  addIf("Earnings", ["earnings", "quarter", "q1", "q2", "q3", "q4", "results", "revenue", "profit", "guidance"])
  addIf("Analyst", ["analyst", "price target", "upgrade", "downgrade", "rating"])
  addIf("M&A", ["acquisition", "acquire", "merger", "buyout", "deal"])
  addIf("Regulation", ["regulation", "antitrust", "lawsuit", "doj", "sec", "eu", "fine"])
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Take only the first 10 most recent articles

...
async function postWebhook(url: string, payload: WebhookPayload, timeoutMs = 2000) {
...
async function buildSnapshot(symbol: string) {
  ...
  const chartUrl = makeUrl("/api/stocks/charts-data", { symbol, interval: "1day", outputsize: "5" })
  const ownershipUrl = makeUrl("/api/stocks/ownership", { symbol })

  const [quoteRes, metricRes, newsRes, chartRes, ownershipRes] = await Promise.all([
  ...
  const ownership = Array.isArray(ownershipRes?.owners) ? ownershipRes.owners.slice(0, 5) : []
  ...
  return {
    ...
    metrics: {
      ...
      peRatio: (() => {
        const pe = pickMetricNumber(["peTTM", "peBasicExclExtraTTM", "peInclExtraTTM", "peNormalizedAnnual"])
        return pe > 0 ? pe : null
      })(),
      week52High: pickMetricNumber(["52WeekHigh", "52WeekHighAdjusted", "week52High"]),
      week52Low: pickMetricNumber(["52WeekLow", "52WeekLowAdjusted", "week52Low"]),
    },
    ...
    ownership: ownership.map((item: any) => ({
      ...
      portfolioPercent: typeof item?.portfolioPercent === "number" ? item.portfolioPercent : 0,
...
async function acquireRefreshLock(supabase: Awaited<ReturnType<typeof createClient>>, symbol: string): Promise<boolean> {
  try {
    ...
    const { error: upsertError } = await supabase
      .from("stock_summary_refresh_locks")
      .upsert({ symbol, refreshing_until: lockUntil }, { onConflict: "symbol" })
    ...
    if (webhookUrl) {
      ...
      if (canTrigger) {
        void (async () => {
          ...
          await postWebhook(webhookUrl, { symbol: normalizedSymbol, reason: "missing", data: snapshot })
        })().catch(() => {})
      }
    }
